...
codeunit 80102 "KLT Document Validator"
{
    var
        ...
        VendorNotExistErr: Label 'Vendor %1 does not exist in BC27';
    ...
    /// <summary>
    /// Validates Purchase Invoice before sending to target
    /// </summary>
    ...
    begin
        ...
        if not Vendor.Get(PurchHeader."Buy-from Vendor No.") then begin
            ErrorText := StrSubstNo(VendorNotExistErr, PurchHeader."Buy-from Vendor No.");
            exit(false);
        end;
    ...
    /// <summary>
    /// Validates Purchase Credit Memo before sending to target
    /// </summary>
